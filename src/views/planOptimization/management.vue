  ...
  <div class="planOptimization-main">
    ...
    <div v-if="selectMenu === 'resultPreview'" class="custom-main-block custom-main-block-result">
      <div class="center-block" id="center-block-resultPreview">
        <a-row class="row">
          <a-button class="left-btn" type="default" @click="handleResultViewModalOpen('MES')">查看MES收率库存结果</a-button>
          <a-button class="left-btn" type="default" @click="handleResultViewModalOpen('DB')">查看DeltaBase结果</a-button>
          ...
          <a-button
            ...
            type="default"
            :disabled="isIntervalStart()"
            @click="handleGenerateMonthScheduleReport">生成月度计划报表
          </a-button>
          <a-upload
          ...
          <file-downloader
            style="margin-top: 11px; margin-right: 0.2rem;"
            buttonType="default"
            size="small"
            down-load-url="/File/DownloadDataBase"
            get-file-url="/Download/Database"
            button-text="下载数据库"/>
          <a-upload
            :showUploadList="false"
          ...
          <div class="custom-date-picker"><span>报表时间：</span><a-month-picker v-model="previewDate" /></div>
        </a-row>
      </div>
      <div class="bottom-block">
        <div class="common-block-title" :style="{padding: '20px 0 10px 20px'}">结果预览</div>
...
export default {
  ...
  methods: {
    ...
    calculationJiChuData () {
      ...
      requestGetJiChuShuJu(params).then(res => {
        if (res.code === 1) {
          const guid = res.data
    ...
    requestJiChuTableData (guid) {
      requestGetTaskLogJiChu({ username: this.username, guid: guid }).then(res => {
    ...
    requestJiaoHeTableData (guid) {
      requestGetTaskLogJiaoHe({ username: this.username, guid: guid }).then(res => {
    ...
    calculationYouHuaData () {
      ...
      requestYouHua({ username: this.username }).then(res => {
        if (res.code === 1) {
          const guid = res.data
    ...
    requestYouHuaTableData (guid) {
      requestGetTaskLogYouHua({ username: this.username, guid: guid }).then(res => {
    ...
    requestResultModalData (type) {
      const params = { type }
      requestGetCache(params).then(res => {
    ...
    handleOpeartePIMSData (action) {
      ...
      requeseOpeartePIMSData(params).then(res => {
        if (res.code === 1) {
          this.$message.success(res.data)
        } else {
          this.$message.error(res.data)
    ...
    handleViewModelRunLog () {
      ...
      requestGetPIMSExecLog(params).then(res => {
        if (res.code === 1) {
    ...
    handleGenerateMonthScheduleReport () {
      ...
      genPlanReport({ username: this.username, month: this.previewDate.format('YYYY-MM') }).then(res => {
        const guid = res.data
