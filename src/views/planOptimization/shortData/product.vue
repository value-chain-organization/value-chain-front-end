<template>
  <div class="short-product-table">
    <a-card :bordered="false" class="top-card">
      <a-row class="row">
        <div class="left">产品表</div>
        <!--
        <div class="right">
    ...
    <a-card :bordered="false" class="table-card">
      <a-table
        ...
        <template slot="title" class="table-title">产品表</template>
...
export default {
  ...
  methods: {
    ...
    getTableData () {
      showTable({ dataType: 'product' }).then(res => {
        // console.log(res);
        const rawData = res.data
        const formatData = []
        let record = { name: '', MIN: {}, MAX: {}, PRICE: {} }
        ...
        const result = [
          ...
          { title: '价格', dataIndex: 'PRICE', width: '25%', scopedSlots: { customRender: 'price' } }
        ]
